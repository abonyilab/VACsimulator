...
%-----------------------------------------------------------------------------
%function test_VAcPlant(minute,OSM,EEM,yopfn,uopfn,spfn)
...
for k=0:(model_sampling_frequency*minute)
    ...
    if rem(time,1) == 0
        time
        setpoint=[setpoint; SP_O2 SP_C2H4 SP_HAc SP_LevelVap SP_PresVap 150 SP_TRCT SP_LevelSep SP_TempSep 16.1026 80 SP_LevelAbs 15.1198 25 0.756 25 SP_CO2 SP_C2H6 SP_FEHE SP_H2OCol SP_TempCol 0.5 0.5 SP_ColButtom 45.845 2.1924];
        %         SP_TRCT
        %         SP_H2OCol
        %         MVs(26)
        %         disturbance_ID
    ...
    if time >= 10
        ...
        % Set the state variables from the OSM
        
        cOS_help = find(OSM(:,1)<=time);
        current_OS = cOS_help(end);
        SP_TRCT = OSM(current_OS,3);
        SP_H2OCol = OSM(current_OS,4);
        MVs(26) = OSM(current_OS,5);
        disturbance_ID = OSM(current_OS,6);
        
        % Set the errors from the EEM
        % Get current errors
%         keyboard
        if ~isempty(EEM)
%             keyboard
            curErr = EEM((time >= EEM(:,1)) & (time <= EEM(:,2)),:);
            % Set state variables according to the given errors
            for i=1:size(curErr,1)
%                 keyboard
                MVs(curErr(i,3)) = curErr(i,4);
            end
        end
    end
